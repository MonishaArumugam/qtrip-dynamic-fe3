...
function addAdventureToDOM(adventures) {
  ...
  adventures?.forEach((eachCard)=>{
    ...
    newDiv.innerHTML = `
      <a href="detail/?adventure=${eachCard.id}" id="${eachCard.id}">
        <div class="card activity-card">
          <span class="category-banner">${eachCard.category}</span>
          <img src=${eachCard.image} class="card-img-top" alt=${eachCard.name} />
          ...
          <div class="text-center d-md-flex flex-row justify-content-between align-items-center">
            ...
            <p class="card-text">$${eachCard.costPerHead}</p>
          </div>
          <div class="text-center d-md-flex flex-row justify-content-between align-items-center mt-1">
            <h5 class="card-title">Duration</h5>
            <p class="card-text">${eachCard.duration} Hours</p>
          </div>
          </div>
        </div>
      </a>
    `;
    divEle.append(newDiv);
  })
...
function filterByDuration(list, low, high) {
  ...
  let finalList = [];  
  finalList = list.filter((val) => val.duration >= low && val.duration <= high);
...
function filterByCategory(list, categoryList) {
  ...
  if(categoryList?.length > 0){
    finalList = list.filter((eachList) => categoryList.includes(eachList.category));
  }else{
    finalList = list;
  }
return finalList
...
function filterFunction(list, filters) {
  ...
  if(filters.category){
    ...
    list = filterByDuration(list,parseInt(durationArr[0]),parseInt(durationArr[1]));
  }
